...
ブラウザーが`127.0.0.1`または`localhost`を開いている場合だけは、
設定が空でも現在のOriginをローカルAPIとして使用します。
本番Cloudflareではこのフォールバックは動作しません。
...
プロフィール設定で固定の管理者ID
`DOOH-IPUT-IS-IDIOT-TEAM-K`を入力すると`admin-login.html`へ移動します。
API設定済みの環境では`POST /admin/identify`でも照合します。
